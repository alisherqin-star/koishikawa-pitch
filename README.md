...
ターミナルで常駐監視する場合（launchd 登録済みなら不要）:
...
launchd に登録すると、ターミナルを閉じても通知が動き続ける。ログイン時にも
自動で再開する。launchd は5分ごとに `--tick` を叩き、tick 自身が「軽い巡回だけか、
全件走査か」を状態ファイルの時刻から判断するので、常駐版と同じ2段構えになる。
