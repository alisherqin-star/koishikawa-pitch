...
**小石川運動場グラウンド**の空き状況を監視し、週末・祝日に空きが出たら Discord に通知する。
依存パッケージなし（Node 20+ の標準機能のみ）。

**すべて GitHub Actions で動く。手元の Mac は不要。**
10分ごとに日単位の軽い巡回を回し、監視対象の日が動いたときだけ全件走査して
Discord に通知し、[公開ページ](https://alisherqin-star.github.io/koishikawa-pitch/)を更新する。
...
普段は何もしなくてよい。以下は手元で確認したいときのコマンド。
...
手元でも常駐監視したい場合（通常は不要。Actions が動いている）:
...
通常は不要 — 監視も通知も Actions 側で完結している。ただし Actions の巡回は
10分間隔なので、それより速く知りたいときは launchd を併走させられる。
5分ごとに `--tick` を叩き、tick 自身が「軽い巡回だけか、全件走査か」を
状態ファイルの時刻から判断する。パスは実行時に埋めるので、テンプレートに
個人のパスは残らない。

併走させるときは `config.json` の `notify.macos` を `true` にし、
**`webhookUrl` は空のままにする**（書くと同じ空きが Discord に2回鳴る）。
...
週末・祝日に空きが出てから通知が届くまでは最短10分（Actions の巡回間隔）。
静音時間帯（23-7 JST）の分は保留され、朝の巡回でまだ空いていれば通知される。
